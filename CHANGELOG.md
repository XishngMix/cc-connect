...
### Added
- **Word filters: `allowed_words` whitelist + per-platform overrides**: new top-level `allowed_words` option (strict whitelist — when non-empty, only messages containing at least one keyword are forwarded to the agent; banned words take precedence), plus `allowed_words_reply` (default `false`: silent block) and `banned_words_reply` (default `true`) reply toggles. Any `[[projects.platforms]]` options block may also set `allowed_words` / `banned_words` and the reply toggles: platform lists are merged with the top-level ones (case-insensitive dedup) and reply toggles override the top-level values for that platform. Applies to all platforms, supports `/reload`.
